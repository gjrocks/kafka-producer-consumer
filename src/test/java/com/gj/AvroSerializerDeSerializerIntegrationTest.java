...
public class AvroSerializerDeSerializerIntegrationTest {
    ...
    @Test
    public void WhenSerializedUsingJSONEncoder_thenObjectGetsSerialized() throws Exception{
        byte[] data = serializer.serializeAvroHttpRequestJSON(request);
        assertTrue(Objects.nonNull(data));
        assertTrue(data.length > 0);
        serializer.serializeAvroHttpRequestJSONToFile(request,"ganesh.avro");
        AvroHttpRequest request1= deserializer.deserializeAvroHttpRequestJSONToFile("ganesh.avro");

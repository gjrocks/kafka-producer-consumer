...
public class EmployeeDataProducerRecordNameStrategy {

    static int delay=1000;
    public static List<RecordMetadata> produce(String broker, String topic, List<Employee> list ) {
        Producer<String, Employee> producer = ProducerCreator.createEmployeeProducerRecordNameStrategy(broker);
        RecordNameStrategy t=null;
        List<RecordMetadata> listRecordMetadata=new ArrayList<>();
        System.out.println("Ganesh here RecordNameStrategy");
        for (Employee employee:list) {
            String key=String.valueOf(employee.getFName());
            System.out.println("Schema name RecordNameStrategy:" +  employee.getSchema().getName());
            System.out.println("Ganesh here-2 RecordNameStrategy");
